...
const emptyStats = { totalOrders: 0, totalProducts: 0, totalUsers: 0, totalRevenue: 0 };
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '5px' }}>
        <img src={logo} alt="Shopify admin logo" style={{ height: '40px', width: '40px', borderRadius: '8px', objectFit: 'cover', filter: 'drop-shadow(0 2px 8px rgba(249, 115, 22, 0.3))' }} />
        <h2 style={{ margin: 0 }}>Admin Dashboard</h2>
      </div>
      <p style={{ color: '#5f6b7a', marginBottom: '30px', fontSize: '1.1rem' }}>
        Welcome back, <strong style={{ color: '#172033' }}>{user?.name}</strong>
        <span style={{ marginLeft: '10px', color: '#c45500', fontSize: '0.85rem', fontWeight: 700 }}>ADMIN</span>
      </p>
      ...
      {stats ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
          ...
          <div style={cardStyle}>
            ...
            <div style={numberStyle}>₹{Number(stats.totalRevenue).toFixed(2)}</div>
          </div>
        </div>
      ) : (
