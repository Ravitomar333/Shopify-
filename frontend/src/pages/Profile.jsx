...
const Profile = () => {
  ...
  const formatOrderDate = (order) => {
    const orderDate = order.createdAt || order.updatedAt || new Date(parseInt(order._id.substring(0, 8), 16) * 1000).toISOString();
    return new Date(orderDate).toLocaleString(undefined, {
      dateStyle: 'medium',
      timeStyle: 'short'
    });
  };
          ...
          {orders.map(order => (
            <div key={order._id} style={{ background: '#09090b', padding: '20px', borderRadius: '12px', border: '1px solid #27272a', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '20px' }}>
              <div>
                ...
                <p style={{ color: '#a1a1aa', fontSize: '0.9rem', marginBottom: '5px' }}>Placed On: <span style={{ color: '#fff' }}>{formatOrderDate(order)}</span></p>
                <p style={{ color: '#a1a1aa', fontSize: '0.9rem' }}>Total: <strong style={{ color: '#10b981' }}>₹{Number(order.totalAmount).toFixed(2)}</strong></p>
              </div>
              <div>
                <span style={{ 
